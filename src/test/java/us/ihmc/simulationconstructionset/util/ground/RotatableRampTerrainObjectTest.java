...
public class RotatableRampTerrainObjectTest
{
   ...
   private Point3D pointsOnSimpleRamp[] = {new Point3D(0, 0, 0), new Point3D(1, 0, 1), new Point3D(0.5, 0, 0.5), new Point3D(0.5, -1, 0.5),
         new Point3D(0.5, 1, 0.5), new Point3D(1, 1, 1), new Point3D(1, -1, 1)};

   private Point3D strictlyInternalPointsOnSimpleRampDown[] = {new Point3D(0.001, 0.0, 0.999), new Point3D(0.999, 0, 0.001), new Point3D(0.5, 0, 0.5),
         new Point3D(0.5, -0.999, 0.5), new Point3D(0.5, 0.999, 0.5), new Point3D(0.999, 0.999, 0.001)};
   ...
   private Point3D pointsOnRamp90[] = {new Point3D(0, 0, 0.5), new Point3D(1, 0, 0.5), new Point3D(-1, 0, 0.5), new Point3D(0, -0.49, 0.01),
         new Point3D(1, -0.5, 0), new Point3D(-0.99, -0.499, 0.001), new Point3D(0.5, 0.25, 0.75), new Point3D(0.9, 0.4, 0.9), new Point3D(1.0, 0.4, 0.9),
         new Point3D(1.0, 0.45, 0.95), new Point3D(1.0, 0.499, 0.999), new Point3D(0, 0.5, 1), new Point3D(-1, 0.5, 1), new Point3D(0.9, 0.5, 1)//, new Point3D(0.909, 0.5,1),//, new Point3D(1, 0.5, 1)

   };

   private Point3D pointsOnRamp90Translated[] = {new Point3D(0, 0, 0.5), new Point3D(-0.99, -0.499, 0.001), new Point3D(0.9, 0.4, 0.9)};

   private Point3D pointsOnRamp90PassingHeightCornerCases[] = {new Point3D(-1, -0.5, 0), new Point3D(0, -0.5, 0)

   };
   private Point3D pointsOnRamp90withNumericalRotationError[] = {new Point3D(0.909, 0.5, 1), new Point3D(1, 0.5, 1)
   ...
   {
      testSurfaceNormalsForAnyOtherRampSides(simpleRamp, expectedSimpleSurfaceNormalOnOtherFaces, pointsOnOtherRampFaces);
   }

   @Test // timeout=300000
   public void testSurfaceNormalAtForSlopedDown()
   {
      testSurfaceNormalsForAnyRampFace(simpleRampDown, expectedSimpleSurfaceNormalSlopeDown, strictlyInternalPointsOnSimpleRampDown);
   }

   @Test // timeout=300000
   public void testOtherSurfaceNormalAtForSlopedDown()
   {
      testSurfaceNormalsForAnyOtherRampSides(simpleRampDown, expectedSimpleSurfaceNormalOnOtherFacesSlopeDown, pointsOnOtherRampFacesSlopeDown);
   }
   ...
   @Disabled //java.lang.AssertionError: Expected Height For point 0.909 0.5 1.0 expected:<1.0> but was:<0.0>
   @Test // timeout=300000
   ...
   {
      testSurfaceNormalsForAnyRampFace(ramp90, expectedSurfaceNormalRamp90, pointsOnRamp90);
   }

   @Test // timeout=300000
   public void testOtherSurfaceNormalForRamp90()
   {
      testSurfaceNormalsForAnyOtherRampSides(ramp90, expectedSurfaceNormalOnOtherFacesRamp90, pointsOnOtherFacesRamp90);
   }
   ...
   private void testHeightAtRampForAnyRampWithTranslation(Point3D[] pointsOnRamp, RotatableRampTerrainObject ramp, Vector3D translation)
   {
      for (int i = 0; i < pointsOnRamp.length; i++)
      {
         String message = "Expected Height For point " + (pointsOnRamp[i].getX() + translation.getX()) + " " + (pointsOnRamp[i].getY() + translation.getY())
               + " " + pointsOnRamp[i].getZ();
         assertEquals(message,
                      pointsOnRamp[i].getZ(),
                      ramp.heightAt(pointsOnRamp[i].getX() + translation.getX(), pointsOnRamp[i].getY() + translation.getY(), pointsOnRamp[i].getZ()),
                      epsilon);
      }
   }
   ...
   private void testSurfaceNormalsForAnyOtherRampSides(RotatableRampTerrainObject ramp, Vector3D[] expectedSurfaceNormalOnOtherFaces,
                                                       Point3D[] pointsOnOtherRampFaces)
   {
